...
TEST_NAME=smoke_teardown_after_failure; ARTIFACT_DIR=$(r4_make_artifact_dir "${TEST_NAME}")
IMAGE=${REISER4_TEARDOWN_IMAGE:-/tmp/reiser4-teardown.img}; MNT=${REISER4_TEARDOWN_MNT:-/tmp/reiser4-teardown-mnt}; SIZE=${REISER4_SMOKE_SIZE:-128M}; DIRPATH=${MNT}/dir
RESULT=FAIL; FAILED_STAGE=none; MKDIR_RESULT=SKIP; MKDIR_ERROR=; MODULE_LOADED_BEFORE=0; MODULE_UNLOADED_AFTER=0; KTXNMGRD_ALIVE_AFTER=0; LOOP_STUCK_AFTER=0; DMESG_DANGER=0
finish(){ local rc=$?; if r4_dmesg_scan final >"${ARTIFACT_DIR}/dmesg-final-scan.txt" 2>&1; then DMESG_DANGER=0; else DMESG_DANGER=1; fi; cat >"${ARTIFACT_DIR}/summary.txt" <<SUMMARY
TEST=${TEST_NAME}
RESULT=${RESULT}
...
trap finish EXIT
fail_stage(){ FAILED_STAGE=$1; shift; printf '%s\n' "$*"; r4_save_state "${ARTIFACT_DIR}" after-failure; r4_save_dmesg "${ARTIFACT_DIR}" before-cleanup; r4_try_cleanup "${ARTIFACT_DIR}"; exit 1; }
...
rm -f "${IMAGE}"; truncate -s "${SIZE}" "${IMAGE}" || fail_stage mkfs "SMOKE_MKFS_FAIL stage=truncate image=\"${IMAGE}\""; if ! out=$(mkfs.reiser4 -y -f "${IMAGE}" 2>&1); then printf '%s\n' "${out}" >"${ARTIFACT_DIR}/mkfs.log"; fail_stage mkfs "SMOKE_MKFS_FAIL error=\"$(r4_quote_msg "${out}")\""; fi; printf '%s\n' "${out}" >"${ARTIFACT_DIR}/mkfs.log"; echo 'SMOKE_MKFS_PASS'
...
if out=$(mkdir "${DIRPATH}" 2>&1); then MKDIR_RESULT=PASS; echo 'SMOKE_MKDIR_PASS unexpected=1'; else MKDIR_RESULT=FAIL; MKDIR_ERROR=${out}; echo 'SMOKE_EXPECTED_MKDIR_FAIL'; printf 'SMOKE_MKDIR_FAIL error="%s"\n' "$(r4_quote_msg "${out}")"; r4_save_state "${ARTIFACT_DIR}" after-failure; r4_save_dmesg "${ARTIFACT_DIR}" after-mkdir-failure; fi
r4_save_dmesg "${ARTIFACT_DIR}" before-cleanup
r4_try_cleanup "${ARTIFACT_DIR}"
r4_module_loaded || MODULE_UNLOADED_AFTER=1; r4_ktxnmgrd_alive && KTXNMGRD_ALIVE_AFTER=1 || true; r4_any_reiser4_loop_exists && LOOP_STUCK_AFTER=1 || true
if [[ ${MODULE_UNLOADED_AFTER} -eq 1 && ${KTXNMGRD_ALIVE_AFTER} -eq 0 && ${LOOP_STUCK_AFTER} -eq 0 ]]; then echo 'SMOKE_TEARDOWN_AFTER_FAIL_PASS'; RESULT=PASS; exit 0; fi
printf 'SMOKE_TEARDOWN_AFTER_FAIL_FAIL module_stuck=%s ktxnmgrd_alive=%s loop_deleted=%s\n' "$([[ ${MODULE_UNLOADED_AFTER} -eq 1 ]] && echo 0 || echo 1)" "${KTXNMGRD_ALIVE_AFTER}" "${LOOP_STUCK_AFTER}"
FAILED_STAGE=teardown_after_failure; exit 1
