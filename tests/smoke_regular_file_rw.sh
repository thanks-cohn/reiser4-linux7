...
TEST_NAME=smoke_regular_file_rw; ARTIFACT_DIR=$(r4_make_artifact_dir "${TEST_NAME}")
IMAGE=${REISER4_RW_IMAGE:-/tmp/reiser4-rw.img}; MNT=${REISER4_RW_MNT:-/tmp/reiser4-rw-mnt}; SIZE=${REISER4_SMOKE_SIZE:-128M}; PAYLOAD=${REISER4_RW_PAYLOAD:-reiser4 regular file payload}
...
if ! out=$(touch "${MNT}/file" 2>&1); then fail_stage file_create "SMOKE_FILE_CREATE_FAIL error=\"$(r4_quote_msg "${out}")\""; fi; echo 'SMOKE_FILE_CREATE_PASS'
if ! out=$(printf '%s\n' "${PAYLOAD}" >"${MNT}/file" 2>&1); then fail_stage file_write "SMOKE_FILE_WRITE_FAIL error=\"$(r4_quote_msg "${out}")\""; fi; sync || fail_stage sync 'SMOKE_SYNC_FAIL'; echo 'SMOKE_FILE_WRITE_PASS'; echo 'SMOKE_SYNC_PASS'
read_back=$(cat "${MNT}/file" 2>"${ARTIFACT_DIR}/file-read.err") || fail_stage file_read "SMOKE_FILE_READ_FAIL error=\"$(r4_quote_msg "$(cat "${ARTIFACT_DIR}/file-read.err")")\""; [[ ${read_back} == "${PAYLOAD}" ]] || fail_stage file_read 'SMOKE_FILE_READ_FAIL mismatch=1'; echo 'SMOKE_FILE_READ_PASS'
umount "${MNT}" || fail_stage unmount 'SMOKE_UNMOUNT_FAIL'; echo 'SMOKE_UNMOUNT_PASS'
if ! out=$(mount -t reiser4 -o loop "${IMAGE}" "${MNT}" 2>&1); then fail_stage remount "SMOKE_REMOUNT_FAIL error=\"$(r4_quote_msg "${out}")\""; fi; echo 'SMOKE_REMOUNT_PASS'
read_back=$(cat "${MNT}/file" 2>"${ARTIFACT_DIR}/remount-read.err") || fail_stage remount_verify "SMOKE_FILE_READ_FAIL after_remount=1 error=\"$(r4_quote_msg "$(cat "${ARTIFACT_DIR}/remount-read.err")")\""; [[ ${read_back} == "${PAYLOAD}" ]] || fail_stage remount_verify 'SMOKE_FILE_REMOUNT_VERIFY_FAIL mismatch=1'; echo 'SMOKE_FILE_REMOUNT_VERIFY_PASS'
umount "${MNT}" || fail_stage unmount2 'SMOKE_UNMOUNT_FAIL phase=after_remount'; echo 'SMOKE_UNMOUNT_PASS phase=after_remount'
