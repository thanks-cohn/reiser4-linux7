								 ...
								 * data */ )
{
	...
	inode_set_mtime_to_ts(object, current_time(object)); inode_set_atime_to_ts(object, current_time(object)); inode_set_ctime_current(object);
...
static int process_truncate(struct inode *inode, __u64 size)
{
	...
	result = inode->i_op->setattr(&nop_mnt_idmap, &dentry, &attr);
	inode_unlock(inode);
